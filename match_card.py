...
class GradientCache:
    ...
    def getCard(self, set_name, name):
        ...
        path = os.path.join(self.base_dir, set_name, name)
        print ("Checking %s" % path)
        if not os.path.isfile(path):
            path = os.path.join(self.base_dir, set_name, name + ".jpg")
        if not os.path.isfile(path):
            path = os.path.join(self.base_dir, set_name, name + ".png")
    ...
    try:
        ...
        while row is not None:
            try:
                ...
                update_c.execute("update inv_cards set name=?, set_name=?, recognition_status=? where rowid=?",
                                 [card, set_name, recognition_status, id])
                connection.commit()
