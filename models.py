...
from elixir import Integer, UnicodeText, BLOB, Enum, DateTime, Boolean, String
...
class InvCard(Entity):
    ...
    box = Field(String(convert_unicode=True))
    scan_png = Field(BLOB)
...
class FixLog(Entity):
    ...
    def __repr__(self):
        return "<card %d was corrected from %s/%s to %s/%s>" % (
        self.card.rowid, self.orig_set, self.orig_name, self.new_set, self.new_name)
