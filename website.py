...
def known_image(set_abbrev, name):
    ...
    sub_path = os.path.join(set_abbrev, name + '.jpg')  # full.
...
def rate_cards():
    ...
    if request.method == 'POST':
        ...
        return render_template('rate.html', box=box, cards=cards,
                               possible_conditions=['mint', 'near_mint', 'good', 'heavy_play'])
...
def reinsert_cards():
    if request.method == 'POST':
        ...
        box_capacity = list(metadata.bind.execute(
            "select box,60 - count(*) as c from inv_cards where box not null and cast( box as int) != 0 group by box having c>0 order by c desc;"))
